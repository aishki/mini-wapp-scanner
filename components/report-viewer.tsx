  ...
  return (
    <div className="space-y-6">
        ...
        <CardHeader>
          <div className="flex items-start justify-between gap-4 flex-wrap">
            ...
            <div className="flex gap-2">
              <Button onClick={handleDownloadJSON} variant="outline" size="sm" className="gap-2 bg-transparent">
                <Download className="w-4 h-4" />
                JSON
              </Button>
              <Button onClick={handleDownloadHTML} variant="outline" size="sm" className="gap-2 bg-transparent">
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div>
              ...
              <p className="text-sm font-medium">{new Date(report.timestamp).toLocaleDateString()}</p>
            </div>
            <div>
          ...
          <div className="grid grid-cols-5 gap-2 pt-4 border-t border-border">
            <div className="text-center">
              <div className="text-2xl font-bold text-red-500">{report.summary.critical}</div>
              <p className="text-xs text-muted-foreground">Critical</p>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-orange-500">{report.summary.high}</div>
              <p className="text-xs text-muted-foreground">High</p>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-yellow-500">{report.summary.medium}</div>
              <p className="text-xs text-muted-foreground">Medium</p>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-blue-500">{report.summary.low}</div>
              <p className="text-xs text-muted-foreground">Low</p>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-cyan-500">{report.summary.total}</div>
      ...
      <div className="space-y-4">
        <h2 className="text-xl font-bold text-foreground">Vulnerabilities Found</h2>
        {report.vulnerabilities.length === 0 ? (
          ...
          report.vulnerabilities.map((vuln) => (
            <Card key={vuln.id} className="border-border bg-card/50 overflow-hidden">
              <CardHeader className="pb-3">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <h3 className="font-semibold text-foreground">{vuln.type}</h3>
              ...
              <CardContent className="space-y-3">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-sm font-mono bg-input p-2 rounded border border-border break-all">{vuln.url}</p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground mb-1">Parameter</p>
                    <p className="text-sm font-mono bg-input p-2 rounded border border-border">{vuln.parameter}</p>
                  </div>
                </div>

                <div>
                  <p className="text-xs text-muted-foreground mb-1">Payload Used</p>
                  <div className="flex gap-2">
