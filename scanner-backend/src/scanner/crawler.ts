    ...
    try {
      ...
      try {
        await page
          ...
          .catch(() => {
            console.log(
              `[crawler] Angular wait timeout for ${url}, continuing...`
            );
          });
      } catch (e) {
        console.log(`[crawler] Angular detection skipped for ${url}`);
      }
      ...
      for (const link of links) {
        ...
        if (href && !href.startsWith("#") && !href.startsWith("javascript:")) {
          ...
          if (this.isSameDomain(absoluteUrl)) {
            console.log(`[crawler] Queuing link: ${absoluteUrl}`);
      ...
      console.log(`[crawler] Found ${forms.length} forms on ${url}`);
      ...
      console.error(`[crawler] Error crawling ${url}:`, err);
    }
  }
